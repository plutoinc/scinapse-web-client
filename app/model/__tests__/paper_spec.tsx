  ...
  describe("PaperStateFactory function", () => {
    ...
    describe("when there are params", () => {
      ...
      it("should return recordified state", () => {
        expect(PaperFactory(mockUserObject).toString()).toContain("Record");
      });

      it("should return same value with params value", () => {
        expect(PaperFactory(mockUserObject).toJS()).toEqual(mockUserObject);
      });

      it("should return same id with params", () => {
        expect(PaperFactory(mockUserObject).id).toEqual(mockId);
      });

      it("should return same cognitivePaperId with params", () => {
        expect(PaperFactory(mockUserObject).cognitivePaperId).toEqual(mockCognitivePaperId);
      });

      it("should return same title with params", () => {
        expect(PaperFactory(mockUserObject).title).toEqual(mockTitle);
      });

      it("should return same year value with params", () => {
        expect(PaperFactory(mockUserObject).year).toEqual(mockYear);
      });

      it("should return same referenceCount value with params", () => {
        expect(PaperFactory(mockUserObject).referenceCount).toEqual(mockReferenceCount);
      });

      it("should return same citedCount with params", () => {
        expect(PaperFactory(mockUserObject).citedCount).toEqual(mockCitedCount);
      });

      it("should return same lang with params", () => {
        expect(PaperFactory(mockUserObject).lang).toEqual(mockLang);
      });

      it("should return same doi with params", () => {
        expect(PaperFactory(mockUserObject).doi).toEqual(mockDoi);
      });

      it("should return same publisher with params", () => {
        expect(PaperFactory(mockUserObject).publisher).toEqual(mockPublisher);
      });

      it("should return same venue with params", () => {
        expect(PaperFactory(mockUserObject).venue).toEqual(mockVenue);
      });

      it("should return recorded fosList with params", () => {
        expect(PaperFactory(mockUserObject).fosList.get(0)).toEqual(FosFactory(initialFos));
      });
      ...
      it("should return same abstract with params", () => {
        expect(PaperFactory(mockUserObject).abstract).toEqual(mockAbstract);
      });

      it("should return same commentCount with params", () => {
        expect(PaperFactory(mockUserObject).commentCount).toEqual(mockCommentCount);
      });

      it("should return recorded comments with params", () => {
        expect(JSON.stringify(PaperFactory(mockUserObject).comments.get(0))).toEqual(
          JSON.stringify(recordifyComment(initialComment)),
        );
      });

      it("should return recorded urls with params", () => {
        expect(PaperFactory(mockUserObject).urls.get(0)).toEqual(PaperSourceFactory(initialPaperSource));
      });

      it("should return recorded journal with params", () => {
        expect(PaperFactory(mockUserObject).journal).toEqual(JournalFactory(mockJournal));
      });
    });
  });
});
